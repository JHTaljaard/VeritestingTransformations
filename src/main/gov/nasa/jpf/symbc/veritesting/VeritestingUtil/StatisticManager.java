...
public class StatisticManager {
    ...
    public void updateVeriSuccForRegion(String key) {
        ...
        if (regionsStatisticsMap.get(key) != null) {
            ...
            regionsStatisticsMap.get(key).fixedPointTime += FixedPointWrapper.getFixedPointTime();
        } else {
            RegionStatistics regionStatistics = new RegionStatistics(key, null, 1, 0, 0, FixedPointWrapper.getFixedPointTime());
    ...
    public void updateSPFHitForRegion(String key, String failError) {
        ...
        if (regionsStatisticsMap.get(key) != null) {
            ...
            regionStatistics = new RegionStatistics(key, null, 0, 1, 0, FixedPointWrapper.getFixedPointTime());
            regionsStatisticsMap.put(key, regionStatistics);
        }
    ...
    public void updateConcreteHitStatForRegion(String key) {
        ...
        if (regionsStatisticsMap.get(key) != null) {
            ...
            regionStatistics = new RegionStatistics(key, null, 0, 0, 1, FixedPointWrapper.getFixedPointTime());
            regionsStatisticsMap.put(key, regionStatistics);
        }
    }
    ...
    public String printAccumulativeStatistics() {
        String out = "\n/************************ Printing Region Statistics *****************\n" +
                ...
                + "\nTotal Time of fixed Point iterations = " + TimeUnit.NANOSECONDS.toMillis(getTotalFixedPointTime()) + "msec";
        return out;
    }
...
//            if (!isInterestingRegion(key)) continue;
    ...
    public long getTotalFixedPointTime() {
        long time = 0;
        Set<String> keys = regionsStatisticsMap.keySet();
        Iterator<String> keysItr = keys.iterator();

        while (keysItr.hasNext())
            time += regionsStatisticsMap.get(keysItr.next()).fixedPointTime;
